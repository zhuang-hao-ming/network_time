...
config = {
    ...
    'new_track_time_table_name': 'new_track_time_5_10_2',
    'output_csv_name': 'validate_result_5_10_2.csv'
}
...
def get_new_tracks():
    ...
    for row in get_all_new_track_time(table_name=config['new_track_time_table_name']):
        id, line_ids, begin_middle, begin_ratio, end_middle, end_ratio, avg_time, _1, _2 = row
        key = tuple(line_ids)
        new_tracks[key] = avg_time
        ...
        for track_seg_id, red_tracks in red_tracks_dict.items(): # 遍历 还原轨迹 - 红灯轨迹
            ...
            for i in range(1, len(red_tracks)):
                ...
                if avg_time is None:
                    ...
                    break
                expect_elapse += new_tracks.get(key)
            ...
            if expect_success:
                ...
                cnt+=1
                csv_writer.writerow([expect_elapse, real_elapse])
    csv_file.close()
    # id_file.close()

...
def get_avg_and_std(time_list):
    ...
    dis = (three_forth - one_fourth) * 3


    top_limit = three_forth + dis
    bottom_limit = one_fourth - dis
...
def main():
    ...
    for key, val in new_track_time_dict.items():
        ...
        if len(time_list) >= 50:
            ...
            mean, std = get_avg_and_std(time_list)

            cnt += 1

            records.append((list(key), val['begin_middle'][0],val['begin_middle'][1],val['end_middle'][0],val['end_middle'][1], mean, std,time_list))

...
将gps log文件导入数据库。解压rar文件后，在脚本中指定解压后文件夹的名称，导入的日期区间，id区间，数据表名。
...
### 功能
对于tracks表中的轨迹进行路径匹配得到在路径上的轨迹。

###

1. 注意无论是观察概率还是转移概率 0 =< p <= 1，对于转移概率，可能出现路径为0的情况，此时转移概率为无穷大，应该把它修正为1
...
有construct_track_all.py, construct_track_true.py两个用于构造完整轨迹的脚本。
all对于构建失败的轨迹，采取打断然后继续构建的策略。
true对于构建失败的轨迹，直接抛弃。

在这个实验中为了保证轨迹的完整性，采用true的脚本。
...
1. 从match_track_table_name中得到所有要还原的轨迹id
2.
...
## calculate_time
###
calculate_time.py: 用一个月份的数据构建新路网
calculate_time_two_month.py: 用两个月份的数据构建新路网
calculat_time_prob: 在构建的路网中加入了概率信息
...
###
validate_result.py: 一般
validate_result_shortest.py: 验证结果包括最短路径
validate_result_prob.py: 验证结果包括概率信息
